locals {
  ...
  egress_rules_to_cidrs = try(length(var.egress_rules["to_cidrs"]["cidrs"]) > 0 ? flatten([
    for cidr in var.egress_rules["to_cidrs"]["cidrs"] : {
      key                       = "egress-${var.egress_rules["to_cidrs"]["protocol"]}-${cidr}"
  ...
  all_rules_map = merge(
    # Ingress rules with auto-incrementing priorities starting at 100
    ...
    var.enable_any_nsg_to_self ? {
      "allow-self-to-self" = {
        ...
        direction                 = "Inbound"
        access                    = "Allow"
        priority                  = 100 + length(local.all_ingress_rules)
      }
    } : {},
    # Egress rules with auto-incrementing priorities starting after ingress
